    ...
    context 'Data de Vencimento' do
      ...
      it 'não pode ser igual à data de realização do evento' do
        ...
        order = Order.create!(event_type: event_type, buffet: buffet, customer: customer, number_of_guests: 80,
                              ...
                              status: :accepted)
        event = Event.new(expiration_date: 2.weeks.from_now, order: order)
      ...
      it 'não pode ser após a data de realização do evento' do
        cash = PaymentMethod.create!(name: 'Dinheiro')
        admin = Admin.create!(email: 'saboresdivinos@email.com', password: 'senha123')
        buffet_photo = Photo.create!()
        buffet_photo.image.attach(io: File.open(Rails.root.join('spec', 'support', 'images', 'buffet_image.jpg')),
        ...
        order = Order.create!(event_type: event_type, buffet: buffet, customer: customer, number_of_guests: 80,
                              estimated_date: 2.weeks.from_now, address: 'Avenida Principal, 100',
                              status: :accepted)
        event = Event.new(expiration_date: 3.weeks.from_now, order: order)
      ...
      it 'deve ser anterior à data de realização do evento' do
        cash = PaymentMethod.create!(name: 'Dinheiro')
        admin = Admin.create!(email: 'saboresdivinos@email.com', password: 'senha123')
        buffet_photo = Photo.create!()
        buffet_photo.image.attach(io: File.open(Rails.root.join('spec', 'support', 'images', 'buffet_image.jpg')),
        ...
        order = Order.create!(event_type: event_type, buffet: buffet, customer: customer, number_of_guests: 80,
                              estimated_date: 2.weeks.from_now, address: 'Avenida Principal, 100',
                              status: :accepted)
        event = Event.new(expiration_date: 1.weeks.from_now, order: order)
